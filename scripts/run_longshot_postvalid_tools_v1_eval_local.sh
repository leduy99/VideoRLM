...
LONGSHOT_CONTEXT_DATASET_NAME="${LONGSHOT_CONTEXT_DATASET_NAME:-${DATASET_NAME:-postvalid_v1}}"
SPLIT="${SPLIT:-test}"
...
CONTROLLER_REPO="${CONTROLLER_REPO:-Qwen/Qwen3.5-9B}"
CONTROLLER_MODEL_PATH="${CONTROLLER_MODEL_PATH:-output/models/Qwen__Qwen3.5-9B}"
CONTROLLER_MAX_NEW_TOKENS="${CONTROLLER_MAX_NEW_TOKENS:-}"
CONTROLLER_TRUST_REMOTE_CODE="${CONTROLLER_TRUST_REMOTE_CODE:-0}"
CONTROLLER_DEVICE_MAP="${CONTROLLER_DEVICE_MAP:-}"
CONTROLLER_GPU_MEMORY="${CONTROLLER_GPU_MEMORY:-}"
CONTROLLER_CPU_MEMORY="${CONTROLLER_CPU_MEMORY:-}"
CONTROLLER_OFFLOAD_FOLDER="${CONTROLLER_OFFLOAD_FOLDER:-output/offload/Qwen__Qwen3.5-9B}"
...
VISUAL_REPO="${VISUAL_REPO:-Qwen/Qwen3.5-9B}"
SPEECH_REPO="${SPEECH_REPO:-openai/whisper-large-v3}"
SPEECH_MODEL_PATH="${SPEECH_MODEL_PATH:-output/models/openai__whisper-large-v3}"
...
SEARCH_TOP_K="${SEARCH_TOP_K:-12}"
MAX_FRONTIER_ITEMS="${MAX_FRONTIER_ITEMS:-12}"
SEARCH_MODE="${SEARCH_MODE:-graph}"

CLIP_DURATION_SECONDS="${CLIP_DURATION_SECONDS:-30}"
SCENE_DURATION_SECONDS="${SCENE_DURATION_SECONDS:-180}"
SEGMENT_DURATION_SECONDS="${SEGMENT_DURATION_SECONDS:-45}"
ENABLE_FINE_SPEECH_WINDOWS="${ENABLE_FINE_SPEECH_WINDOWS:-1}"
FINE_SPEECH_WINDOW_SECONDS="${FINE_SPEECH_WINDOW_SECONDS:-15}"
FINE_SPEECH_WINDOW_STRIDE_SECONDS="${FINE_SPEECH_WINDOW_STRIDE_SECONDS:-5}"
...
PITOME_FRAME_EXTRACTION_STRATEGY="${PITOME_FRAME_EXTRACTION_STRATEGY:-sequence}"
...
PITOME_MAX_SCENE_BOUNDARY_FRAMES="${PITOME_MAX_SCENE_BOUNDARY_FRAMES:-0}"
PITOME_SCENE_SAMPLE_RATE="${PITOME_SCENE_SAMPLE_RATE:-0}"
PITOME_SCENE_KEYFRAMES_ONLY="${PITOME_SCENE_KEYFRAMES_ONLY:-1}"
PITOME_EDGE_BOUNDARY_FRAMES="${PITOME_EDGE_BOUNDARY_FRAMES:-0}"
VISUAL_INDEX_BATCH_SIZE="${VISUAL_INDEX_BATCH_SIZE:-64}"
VISUAL_INDEX_WORKERS="${VISUAL_INDEX_WORKERS:-4}"
...
ENABLE_CONTROLLER_EVIDENCE_CLASSIFIER="${ENABLE_CONTROLLER_EVIDENCE_CLASSIFIER:-0}"
...
ENABLE_ASR_EMBEDDING_RETRIEVAL="${ENABLE_ASR_EMBEDDING_RETRIEVAL:-1}"
SPEECH_EMBEDDING_REPO="${SPEECH_EMBEDDING_REPO:-sentence-transformers/all-MiniLM-L6-v2}"
SPEECH_EMBEDDING_MODEL_PATH="${SPEECH_EMBEDDING_MODEL_PATH:-}"
SPEECH_EMBEDDING_DEVICE="${SPEECH_EMBEDDING_DEVICE:-cuda:0}"
ENABLE_TARGETED_ASR_REFINEMENT="${ENABLE_TARGETED_ASR_REFINEMENT:-1}"
ENABLE_DYNAMIC_EVIDENCE_RETRIEVAL="${ENABLE_DYNAMIC_EVIDENCE_RETRIEVAL:-1}"
ENABLE_REFINEMENT_FRONTIER="${ENABLE_REFINEMENT_FRONTIER:-0}"
DISABLE_QUESTION_ROUTING="${DISABLE_QUESTION_ROUTING:-1}"
...
OFFLOAD_COMPONENTS_AFTER_USE="${OFFLOAD_COMPONENTS_AFTER_USE:-1}"

SKIP_SPEECH_RECOGNITION="${SKIP_SPEECH_RECOGNITION:-0}"
SPEECH_BACKEND="${SPEECH_BACKEND:-faster-whisper}"
LAZY_SPEECH_REFINEMENT="${LAZY_SPEECH_REFINEMENT:-1}"
SPEECH_CHUNK_DURATION_SECONDS="${SPEECH_CHUNK_DURATION_SECONDS:-300}"
SPEECH_MAX_NEW_TOKENS="${SPEECH_MAX_NEW_TOKENS:-1536}"
SPEECH_ASR_CHUNK_BATCH_SIZE="${SPEECH_ASR_CHUNK_BATCH_SIZE:-12}"
FASTER_WHISPER_MODEL="${FASTER_WHISPER_MODEL:-large-v3}"
...
FASTER_WHISPER_CHUNK_WORKERS="${FASTER_WHISPER_CHUNK_WORKERS:-4}"
...
CONTROLLER_OFFLOAD_ENABLED=0
if [[ -n "${CONTROLLER_DEVICE_MAP}" || -n "${CONTROLLER_GPU_MEMORY}" || -n "${CONTROLLER_CPU_MEMORY}" ]]; then
  CONTROLLER_OFFLOAD_ENABLED=1
elif [[ -n "${CONTROLLER_OFFLOAD_FOLDER}" ]] \
  && is_true "${ENABLE_VIDEO_WINDOW_RERANKING}" \
  && is_true "${OFFLOAD_COMPONENTS_AFTER_USE}"; then
...
ARTIFACTS_DIR="${ARTIFACTS_DIR:-${OUTPUT_ROOT}/artifacts}"
MEMORY_DIR="${MEMORY_DIR:-${OUTPUT_ROOT}/memories}"
TRACE_DIR="${TRACE_DIR:-${OUTPUT_ROOT}/traces}"
...
if is_true "${RUN_INFERENCE}"; then
  cmd=(
    ...
    --longshot-context-dataset-name "${LONGSHOT_CONTEXT_DATASET_NAME}"
    --split "${SPLIT}"
    ...
    --speech-asr-chunk-batch-size "${SPEECH_ASR_CHUNK_BATCH_SIZE}"
    ...
    --faster-whisper-batch-size "${FASTER_WHISPER_BATCH_SIZE}"
    --faster-whisper-chunk-workers "${FASTER_WHISPER_CHUNK_WORKERS}"
    ...
    --fine-speech-window-seconds "${FINE_SPEECH_WINDOW_SECONDS}"
    --fine-speech-window-stride-seconds "${FINE_SPEECH_WINDOW_STRIDE_SECONDS}"
    ...
    --visual-index-workers "${VISUAL_INDEX_WORKERS}"
  )
  if [[ -n "${SAMPLE_LIMIT}" ]]; then
    cmd+=(--sample-limit "${SAMPLE_LIMIT}")
  fi
  if [[ -n "${SAMPLE_START_INDEX}" ]]; then
    cmd+=(--sample-start-index "${SAMPLE_START_INDEX}")
  fi
  if [[ -n "${SAMPLE_END_INDEX}" ]]; then
    cmd+=(--sample-end-index "${SAMPLE_END_INDEX}")
  fi
  ...
  if [[ -n "${SPEECH_MODEL_PATH}" ]]; then
    cmd+=(--speech-model-path "${SPEECH_MODEL_PATH}")
  fi
  if [[ -n "${CONTROLLER_MAX_NEW_TOKENS}" ]]; then
    cmd+=(--controller-max-new-tokens "${CONTROLLER_MAX_NEW_TOKENS}")
  fi
  if [[ -n "${CONTROLLER_DEVICE_MAP}" ]]; then
    cmd+=(--controller-device-map "${CONTROLLER_DEVICE_MAP}")
  fi
  if [[ -n "${CONTROLLER_GPU_MEMORY}" ]]; then
    cmd+=(--controller-gpu-memory "${CONTROLLER_GPU_MEMORY}")
  fi
  if [[ -n "${CONTROLLER_CPU_MEMORY}" ]]; then
    cmd+=(--controller-cpu-memory "${CONTROLLER_CPU_MEMORY}")
  fi
  if [[ "${CONTROLLER_OFFLOAD_ENABLED}" == "1" && -n "${CONTROLLER_OFFLOAD_FOLDER}" ]]; then
    cmd+=(--controller-offload-folder "${CONTROLLER_OFFLOAD_FOLDER}")
  fi
  ...
  if ! is_true "${PITOME_EDGE_BOUNDARY_FRAMES}"; then
    cmd+=(--no-pitome-edge-boundary-frames)
  fi
  if is_true "${SKIP_SPEECH_RECOGNITION}"; then
    cmd+=(--no-speech-recognition)
  elif is_true "${LAZY_SPEECH_REFINEMENT}"; then
    cmd+=(--lazy-speech-refinement)
  else
    cmd+=(--eager-speech-recognition)
  fi
  cmd+=(--lazy-visual-refinement)
  if is_true "${ENABLE_CONTROLLER_EVIDENCE_CLASSIFIER}"; then
    cmd+=(--enable-controller-evidence-classifier)
  fi
  if is_true "${OFFLOAD_COMPONENTS_AFTER_USE}"; then
    cmd+=(--offload-components-after-use)
  fi
  if is_true "${ENABLE_TARGETED_ASR_REFINEMENT}"; then
    cmd+=(--enable-targeted-asr-refinement)
  fi
  if ! is_true "${ENABLE_DYNAMIC_EVIDENCE_RETRIEVAL}"; then
    cmd+=(--disable-dynamic-evidence-retrieval)
  fi
  if is_true "${DISABLE_QUESTION_ROUTING}"; then
    cmd+=(--disable-question-routing)
  fi
  if ! is_true "${ENABLE_REFINEMENT_FRONTIER}"; then
    cmd+=(--disable-refinement-frontier)
  fi
  ...
  if is_true "${ENABLE_FINE_SPEECH_WINDOWS}"; then
    cmd+=(--enable-fine-speech-windows)
  fi
  if is_true "${ENABLE_ASR_EMBEDDING_RETRIEVAL}"; then
    cmd+=(
      --speech-embedding-repo "${SPEECH_EMBEDDING_REPO}"
      --speech-embedding-device "${SPEECH_EMBEDDING_DEVICE}"
    )
    if [[ -n "${SPEECH_EMBEDDING_MODEL_PATH}" ]]; then
      cmd+=(--speech-embedding-model-path "${SPEECH_EMBEDDING_MODEL_PATH}")
    fi
  fi
  ...
  dataset_label="${DATASET_PATH}${DATASET_NAME:+/${DATASET_NAME}}:${SPLIT}"
  echo "[LongShOT] running ${dataset_label} context=${LONGSHOT_CONTEXT_DATASET_NAME}" >&2
